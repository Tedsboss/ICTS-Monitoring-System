            ...
            <section class="border-b border-slate-200 p-4">
                ...
                <div class="mb-3 flex flex-wrap items-center justify-between gap-3">
                    ...
                    <div>
                        ...
                        <p class="mb-0 mt-1 text-xs text-slate-500">MOOE, Capital Outlay and NINP allocation balances.</p>
                    ...
                    @foreach ([
                        ...
                        <div class="rounded-xl border border-slate-200 bg-slate-50 p-3">
                            ...
                            <div class="mb-2 flex items-center justify-between gap-2">
                                <div>
                                    <div class="text-xs font-bold uppercase tracking-wide text-slate-700">{{ $label }}</div>
                                    <div class="mt-0.5 text-[11px] text-slate-500">{{ $groupLabel }}</div>
                                </div>
                            </div>
                            ...
                            <input type="number" id="{{ $inputId }}" min="0" step="0.01"

                                   class="builder-write-field w-full rounded-lg border border-slate-300 bg-white px-3 py-2 text-sm font-semibold outline-none focus:border-sky-500 focus:ring-2 focus:ring-sky-100">
                            ...
                            <div class="mt-3 grid grid-cols-2 gap-2">
                                ...
                                <div class="rounded-lg border border-slate-200 bg-white px-3 py-2">
                                    ...
                                    <div class="mt-1 text-sm font-bold text-slate-700"
                                         id="{{ $originalProgrammedId }}">0.00</div>
                                </div>
                                ...
                                <div class="rounded-lg border border-emerald-200 bg-emerald-50 px-3 py-2">
                                    ...
                                    <div class="mt-1 text-sm font-bold text-emerald-800"
                                         id="{{ $savingsId }}">0.00</div>
                                </div>

                                <div class="rounded-lg border border-sky-200 bg-sky-50 px-3 py-2">
                                    <div class="text-[10px] font-semibold uppercase tracking-wide text-sky-700">
                                        Remaining Balance
                                    </div>
                                    <div class="mt-1 text-sm font-bold text-sky-900"
                                         id="{{ $balanceId }}">0.00</div>
                                </div>
...
$(document).ready(function () {
    ...
    function applyLockState(locked, status = 'draft') {
        ...
        $badge
            ...
            .addClass(locked ? 'bg-success' : 'bg-secondary')

            .text(locked ? 'Finalized / Locked' : label);
        ...
        $('#builderBody .row-type-select, #builderBody .field-input, #builderBody .month-input')
            ...
            .prop('disabled', function () {
                ...
                if ($(this).hasClass('row-type-select')) {

                    return false;

                }

                if ($(this).data('field') === 'program_classification') {
        ...
        $('#builderBody .program-classification-same')
            .prop('disabled', locked);
    ...
    // Calculate live programmed totals
    ...
    function getLiveTotals() {
        ...
        let originalMooeSum = 0;
        let originalCoSum = 0;
        let originalNinpMooeSum = 0;
        ...
        $('#builderBody tr').each(function () {
            ...
            // Allocation Type, not PREXC, decides which allocation balance is used.
            // The master record also decides which expense categories are allowed.
            ...
            const allowsCapitalOutlay = Boolean(Number(allocationMaster.allows_capital_outlay));

            // The current allocation balance cards/table are still the ICTS structure:
            // MITHI MOOE, MITHI Capital Outlay, and NINP MOOE.
            // Do not infer unknown Staff/Office allocation types into these buckets.
            if (allocationType === 'mithi') {
                if (allowsMooe) {
                    originalMooeSum += mooe;
                    mooeSum += effectiveMooe;
                }

                if (allowsCapitalOutlay) {
                    originalCoSum += co;
                    coSum += effectiveCo;
                }
            }

            if (allocationType === 'ninp' && allowsMooe) {
                originalNinpMooeSum += mooe;
                ninpMooeSum += effectiveMooe;
            }
    ...
    // Recalculate allocation balances
    ...
    function recalcLiveBalance() {
        ...
        const mooeAllocation = parseFloat($('#mooeAllocation').val()) || 0;
        const coAllocation = parseFloat($('#coAllocation').val()) || 0;
        const ninpAllocation = parseFloat($('#ninpAllocation').val()) || 0;

        const mooeSavings = Math.max(0, originalMooeSum - mooeSum);
        const coSavings = Math.max(0, originalCoSum - coSum);
        const ninpSavings = Math.max(0, originalNinpMooeSum - ninpMooeSum);

        // Remaining Balance uses Effective Programmed amounts.
        // Contract savings therefore return automatically to the available balance.
        const mooeBalance = mooeAllocation - mooeSum;
        const coBalance = coAllocation - coSum;
        const ninpBalance = ninpAllocation - ninpMooeSum;

        $('#mooeOriginalProgrammedOut').text(fmtNum(originalMooeSum));
        $('#coOriginalProgrammedOut').text(fmtNum(originalCoSum));
        $('#ninpOriginalProgrammedOut').text(fmtNum(originalNinpMooeSum));

        $('#mooeProgrammedOut').text(fmtNum(mooeSum));
        $('#coProgrammedOut').text(fmtNum(coSum));
        $('#ninpProgrammedOut').text(fmtNum(ninpMooeSum));

        $('#mooeSavingsOut').text(fmtNum(mooeSavings));
        $('#coSavingsOut').text(fmtNum(coSavings));
        $('#ninpSavingsOut').text(fmtNum(ninpSavings));

        $('#mooeBalanceOut')
            .text(fmtNum(mooeBalance))
            .toggleClass('text-danger', mooeBalance < 0);

        $('#coBalanceOut')
            .text(fmtNum(coBalance))
            .toggleClass('text-danger', coBalance < 0);

        $('#ninpBalanceOut')
            .text(fmtNum(ninpBalance))
            .toggleClass('text-danger', ninpBalance < 0);
    ...
    function loadStatus() {
        ...
        return $.getJSON(
            ...
            applyLockState(response.finalized === 'yes', response.status || 'draft');
    ...
    function loadAllocationAndBalance() {
        ...
        return $.getJSON(
            ...
            $('#mooeAllocation').val(response.mooe_allocation ?? 0);

            $('#coAllocation').val(response.capital_outlay_allocation ?? 0);

            $('#ninpAllocation').val(response.ninp_allocation ?? 0);
    ...
    function saveAllocation() {
        ...
        const values = [
            ...
            parseFloat($('#mooeAllocation').val()) || 0,

            parseFloat($('#coAllocation').val()) || 0,

            parseFloat($('#ninpAllocation').val()) || 0
    ...
    function loadPlan(manualReload = false) {
        ...
        $.when(loadStatus(), loadSignatories(), loadAllocationAndBalance()).always(function () {
            ...
            activeLoadRequest = $.getJSON(
                ...
                applyLockState(isLocked, isLocked ? 'finalized' : 'draft');
    ...
    function getSubmissionReadiness() {
        ...
        const { mooeBalance, coBalance, ninpBalance } = getLiveBalancesForReadiness();

        if (mooeBalance < -0.01) {
            issues.push({
                type: 'error',
                message: `MITHI MOOE exceeds the available allocation by ${fmtNum(Math.abs(mooeBalance))}.`
            });
        }
        ...
        if (coBalance < -0.01) {
            issues.push({
                type: 'error',
                message: `MITHI Capital Outlay exceeds the available allocation by ${fmtNum(Math.abs(coBalance))}.`
            });
        }
        ...
        if (ninpBalance < -0.01) {
            issues.push({
                type: 'error',
                message: `NINP exceeds the available allocation by ${fmtNum(Math.abs(ninpBalance))}.`
            });
        }
    ...
    function getLiveBalancesForReadiness() {
        ...
        return {
            mooeBalance: mooeAllocation - mooeSum,
            coBalance: coAllocation - coSum,
            ninpBalance: ninpAllocation - ninpMooeSum
        };
    }
    ...
    $('#mooeAllocation, #coAllocation, #ninpAllocation').on('input', function () {
