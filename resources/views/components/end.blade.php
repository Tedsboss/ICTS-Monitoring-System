<div class="fixed-plugin align-middle" id="divSaveFinal" onclick="submitForm('{{ $url }}', '{{ $code }}', '{{ $swal }}', '{{ $swalTitle}}', '{{ $swalText }}')" >
  <a data-bs-toggle="tooltip" style="background-color: #ef5358 !important; bottom: {{ isset($btnPosition) ? $btnPosition : '8' }}vh !important;" class="fixed-plugin-button text-bold text-xs text-center font-weight-bolder position-fixed p-2 fixed-plugin-btn opacity-7 align-middle fixed-plugin-btn-custom">
