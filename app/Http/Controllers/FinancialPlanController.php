...
class FinancialPlanController extends Controller
{
    ...
    private function validateFinancialPlanForSubmit(
        ...
        // Submit works from rows already stored in the database.
        // New or intentionally changed PREXC pairs are enforced by validatePrexcRows()
        // during Save. Existing stored legacy pairs remain valid for submission as
        // long as both Program Classification and PREXC Code are present.
        ...
        foreach ($items as $item) {
            ...
            // An active pair is preferred, but an existing stored legacy pair is
            // intentionally accepted here. If the user changes either value,
            // validatePrexcRows() requires the new pair to match the active master
            // before that change can be saved.
            ...
            if (! $isActivePrexcPair && $classification !== '' && $prexcCode !== '') {
                // Legacy pair: no submit error. Presence is sufficient.
            }
            ...
            // A budget line must have an original programmed amount.
            if ($originalBudget <= 0) {
                $errors["financial_plan_row_{$item->id}_budget"] =
                    "{$prefix}: Enter an MOOE or Capital Outlay amount greater than zero.";
            }

            // Contract Amount replaces the programmed total for financial targeting.
            // It cannot create a negative savings amount.
            ...
            // Use a one-cent tolerance for stored decimal values.
            ...
            // Current ICTS allocation balance columns are still MITHI MOOE,
            // MITHI Capital Outlay, and NINP. PREXC is never used here.
            // Other Staff/Office balance structures must be configured before
            // their allocation calculations are enabled.
            if ($allocationType === 'mithi') {
                $mooeSum += $effectiveMooe;
                $coSum += $effectiveCo;
            }

            if ($allocationType === 'ninp') {
                $ninpSum += $effectiveMooe;
            }
        }

        $allocationQuery = FinancialPlanAllocation::query()
            ->where('fiscal_year', $fiscalYear)
            ->where('office_name', $officeName);
        ...
        if ($staffId !== null) {
            ...
            $this->applyStaffScope($allocationQuery);
        }
        ...
        $ninpBalance = $ninpAllocation - $ninpSum;

        if ($mooeBalance < -0.01) {
            $errors['financial_plan_mithi_mooe_balance'] =
                'MITHI MOOE programmed amount exceeds the available allocation by '
                . number_format(abs($mooeBalance), 2) . '.';
        }

        if ($coBalance < -0.01) {
            $errors['financial_plan_mithi_co_balance'] =
                'MITHI Capital Outlay programmed amount exceeds the available allocation by '
                . number_format(abs($coBalance), 2) . '.';
        }

        if ($ninpBalance < -0.01) {
            $errors['financial_plan_ninp_balance'] =
                'NINP programmed amount exceeds the available allocation by '
                . number_format(abs($ninpBalance), 2) . '.';
        }
    ...
    public function builder(Request $request): View
    {
        ...
        if ($officeName !== '') {
            $accessPlan = $this->authorizePlanWrite($fiscalYear, $officeName);
        } else {
            $this->authorize('create', FinancialPlan::class);
        }
    ...
    public function saveSignatories(Request $request): JsonResponse
    {
        ...
        $signatory = FinancialPlanSignatory::updateOrCreate(
            [
                'fiscal_year' => $year,
                'office_name' => $office,
                'staff_id' => $accessPlan?->staff_id ?? auth()->user()->staff_id,
            ],
            [
                'staff_id' => $accessPlan?->staff_id ?? auth()->user()->staff_id,
                'division_id' => $accessPlan?->division_id ?? auth()->user()->division_id,
    ...
    public function save(Request $request): JsonResponse
    {
        ...
        $validated = $request->validate([
            ...
            'rows.*.allocation_type'        => ['nullable', 'string', 'in:mithi,ninp'],
    ...
    public function allocation(Request $request): JsonResponse
    {
        ...
        $allocationQuery = FinancialPlanAllocation::where('fiscal_year', $fiscalYear)
            ->where('office_name', $officeName);
    ...
    public function totals(Request $request): JsonResponse
    {
        ...
        // Resolve the Staff/Office owner before loading Allocation Type master data.
        // Administrators may have no staff_id, so the Financial Plan owner is authoritative.
        ...
        $items = $itemsQuery->get(['mooe', 'capital_outlay', 'contract_amount', 'allocation_type']);

        $allocationTypes = collect();

        if ($staffId !== null) {
            $allocationTypes = FinancialPlanAllocationType::query()
                ->forStaff((int) $staffId)
                ->active()
                ->get([
                    'code',
                    'allows_mooe',
        ...
        foreach ($items as $row) {
            ...
            // Expense-category permissions come from the Staff/Office master data.
            ...
            // The current allocation balance columns are still the ICTS structure:
            // MITHI MOOE, MITHI Capital Outlay, and NINP MOOE.
            // Unknown Staff/Office allocation types are never inferred into these buckets.
            if ($allocationType === 'mithi') {
                if ($allocationMaster->allows_mooe) {
                    $mooeSum += $effMooe;
                }

                if ($allocationMaster->allows_capital_outlay) {
                    $coSum += $effCo;
                }
            }

            if ($allocationType === 'ninp' && $allocationMaster->allows_mooe) {
                $ninpSum += $effMooe;
            }
        }
        ...
        if ($accessPlan?->staff_id !== null) {
            $allocationQuery->where('staff_id', $accessPlan->staff_id);
        } else {
            $this->applyStaffScope($allocationQuery);
        }
    ...
    public function submitForApproval(Request $request): JsonResponse
    {
        ...
        // Make sure the plan exists inside the current staff scope.
        ...
        $oldStatus = $this->getSubmission($year, $office, $plan->staff_id)?->status ?? 'draft';
        ...
        // Apply the complete Financial Plan validation before changing workflow status.
        $this->validateFinancialPlanForSubmit($year, $office, $plan->staff_id);
    ...
    public function approve(Request $request): JsonResponse
    {
        ...
        $year = (int) $validated['fiscal_year'];
        $office = $validated['office_name'];
        $submission = $this->getSubmission($year, $office);
        ...
        $plan = FinancialPlan::where('fiscal_year', $year)
            ->where('office_name', $office)
            ->first();
    ...
    {
        $this->authorize('return', FinancialPlan::class);
    ...
    {
        $this->authorize('finalize', FinancialPlan::class);
        ...
        $existingSubmission = $this->getSubmission($year, $office);
    ...
    {
        $this->authorize('reopen', FinancialPlan::class);
    ...
    public function status(Request $request): JsonResponse
    {
        ...
        $submissionQuery = FinancialPlanSubmission::where('fiscal_year', $validated['fiscal_year'])
            ->where('office_name', $validated['office_name']);

        if ($plan?->staff_id !== null) {
            $submissionQuery->where('staff_id', $plan->staff_id);
        } else {
        ...
        $canApprove = auth()->user()->can('approve', FinancialPlan::class);
        $canReturn = auth()->user()->can('return', FinancialPlan::class);
        $canFinalize = auth()->user()->can('finalize', FinancialPlan::class);
        $canReopen = auth()->user()->can('reopen', FinancialPlan::class);
        ...
        return response()->json([
            ...
            'can_reopen' => $finalized === 'yes'
                && $canReopen,
            'submitted_by' => $displayName($submission?->submittedBy),
            'submitted_at' => $submission?->submitted_at,
            'approved_by' => $displayName($submission?->approvedBy),
            'approved_at' => $submission?->approved_at,
            'finalized_by' => $displayName($submission?->finalizedBy),
